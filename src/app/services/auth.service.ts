...
export class AuthService {
  ...
  constructor(private api: AdminApiService, private alert: AlertService, private router: Router, private studentapi:StudentApiService ) { }
  ...
  loginStudent(data: LoginDTO) {
    this.studentapi.post('/apistudent/validate/student/login', data).subscribe((res: any) => {
  ...
  loginAdmin(data: LoginDTO) {
    this.api.post('/apiadmin/validate/admin/login', data).subscribe((res: any) => {
      if (!res?.response?.email) {
        this.alert.error("Wrong credentials");
        return;
      }
  ...
  getLoggedInStudent() {
    ...
    return this.studentapi.get(`/apistudent/admin/view-student-by-id/${Id}`);
  }
  ...
  getLoggedInAdmin() {
    ...
    const Id = sessionStorage.getItem('SESSION_USER_ID');
    return this.api.get(`/apiadmin/admin/view-admin-by-id/${Id}`);
  }

  updateStudent(credentials: StudentRegisterDTO, callback?: () => void) {
    this.studentapi.put(`/apistudent/student/update-student/${credentials.id}`, credentials).subscribe(res => {
  ...
  updateAdmin(credentials: AdminRegisterDTO, callback?: () => void) {
    this.api.put(`/apiadmin/update-admin/${credentials.id}`, credentials).subscribe(res => {
      this.alert.success('Update successful.');
      let userRef:any = res;
      userRef.password= "";
  ...
  deleteStudent(Id: number, callback?: () => void) {
    this.studentapi.delete(`/apistudent/admin/delete-student/${Id}`).subscribe(res => {
